...
#         self.shared.rawdatabase_sensor = pd.concat(dfs,ignore_index=True)
#         self.shared.database_sensor=self.shared.rawdatabase_sensor[self.shared.rawdatabase_sensor['Longitude'].notnull()]
...
class ACETI_GUI(tkinter.Tk):
    def __init__(self):
        ...
        self.mainFrame.add(self.toolbar)
        self.menubar_buttons = []
        self.button_trash_detection = tkinter.Button(self.toolbar, image=self.assets.icon_water_trash,
                                                     command=lambda a=0: self.change_tab(a), width=40, height=40)
        self.menubar_buttons.append(self.button_trash_detection)
        self.button_raw_trash_detection = tkinter.Button(self.toolbar, image=self.assets.icon_trash_log,
                                                         command=lambda a=1: self.change_tab(a), width=40, height=40)
        self.menubar_buttons.append(self.button_raw_trash_detection)
        self.button_sensor_detection = tkinter.Button(self.toolbar, image=self.assets.sensor_icon,
                                                      command=lambda a=2: self.change_tab(a), width=40, height=40)
        self.menubar_buttons.append(self.button_sensor_detection)
        for i in self.menubar_buttons:
            i.pack(side='left', expand=False, )
    ...
    def read_database(self, event=None):
        ...
        choice = tkinter.simpledialog.askstring("Select Data Type", "Enter 'trash' to process trash data or 'sensor' to process sensor data:")
        ...
        if choice == 'trash':
            ...
            self.shared.rawdatabase = pd.concat(dfs, ignore_index=True)
            self.shared.database = self.shared.rawdatabase[self.shared.rawdatabase['Longitude'].notnull()]
            self.SENSORTAB.update_database()
        else:
            tkinter.messagebox.showerror("Invalid Choice", "Invalid choice. Please enter 'trash' or 'sensor'.")
            return False
